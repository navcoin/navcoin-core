                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck, bool fProofOfStake)
{
    ...
    // BIP16 didn't become active until Apr 1 2012 (always active for DeVault)
    unsigned int flags = SCRIPT_VERIFY_P2SH;

    // Start enforcing the DERSIG (BIP66) rules
    flags |= SCRIPT_VERIFY_DERSIG;
  
    // Start enforcing CHECKLOCKTIMEVERIFY
...
{
    LOCK(cs_main);
    return (true); // Disable/Remove if we drop SEGWIT
}
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check timestamp
    if (block.GetBlockTime() > nAdjustedTime +  Params().GetConsensus().nMaxFutureDrift)
