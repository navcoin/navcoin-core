...
UniValue Navtech::CreateAnonTransaction(string address, CAmount nValue, int nTransactions) {
  ...
  if (encryptionSuccess) {
    ...
    UniValue dvttechData;
    dvttechData.setObject();
    dvttechData.pushKV("anondestination", encryptedAddress);
    dvttechData.pushKV("public_key", pubKey);
    dvttechData.pushKV("anonaddress", addrArray);
    dvttechData.pushKV("anonfee", find_value(serverData, "transaction_fee"));
  ...
  if (curl) {
    ...
    if (res != CURLE_OK) {
      ...
      if (!data_obj.isObject()) {
        ...
        UniValue dvttechData;
        dvttechData.setObject();
        dvttechData.pushKV("min_amount", find_value(data, "min_amount"));
        dvttechData.pushKV("max_amount", find_value(data, "max_amount"));
        dvttechData.pushKV("transaction_fee", find_value(data, "transaction_fee"));
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        return dvttechData;
      }
