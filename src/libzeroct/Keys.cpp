...
bool CPrivateSpendKey::GetObfuscationJ(libzeroct::ObfuscationValue& obfuscationJ) const {
    if (this->params->initialized == false) return false;
    obfuscationJ = std::make_pair(CBigNum(oj1),CBigNum(oj2));
    return true;
}
bool CPrivateSpendKey::GetObfuscationK(libzeroct::ObfuscationValue& obfuscationK) const {
    if (this->params->initialized == false) return false;
...
bool CPrivateViewKey::GetBlindingCommitment(libzeroct::BlindingCommitment& blindingCommitment) const {
    if (this->params->initialized == false) return false;
...
bool CPrivateAddress::GetBlindingCommitment(libzeroct::ObfuscationValue& blindingCommitment) const {
    if (this->params->initialized == false) return false;
    blindingCommitment = std::make_pair(CBigNum(bc1),CBigNum(bc2));
    return true;
}
