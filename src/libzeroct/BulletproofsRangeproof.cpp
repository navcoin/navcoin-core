...
CBN_vector ip12 = {InnerProduct(VectorSlice(oneN, 0, 64), VectorSlice(twoN, 0, 64)),
                             InnerProduct(VectorSlice(oneN, 0, 128), VectorSlice(twoN, 0, 128)),
                             InnerProduct(VectorSlice(oneN, 0, 256), VectorSlice(twoN, 0, 256)),
                             InnerProduct(VectorSlice(oneN, 0, 512), VectorSlice(twoN, 0, 512))};

void BulletproofsRangeproof::Prove(CBN_vector v, CBN_vector gamma, const size_t logN)
{
    if (pow(2, logN) > maxN)
...
bool VerifyBulletproof(const libzeroct::IntegerGroupParams* params, const std::vector<BulletproofsRangeproof>& proofs, CBN_matrix v, unsigned int logN)
{
    if (proofs.size() == 0)
    ...
    if (pow(2, logN) > BulletproofsRangeproof::maxN)
        throw std::runtime_error("BulletproofsRangeproof::Prove(): logN value is too high");
