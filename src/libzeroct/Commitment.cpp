...
Commitment::Commitment(const IntegerGroupParams* p, const CBigNum& bnSerial, const CBigNum& bnRandomness, const CBigNum& extra): params(p), contents(extra) {
    this->randomness = bnSerial;
    this->extra = bnRandomness;
    this->commitmentValue = (params->g.pow_mod(this->randomness, params->modulus).mul_mod(
        params->h.pow_mod(this->extra, params->modulus), params->modulus)).mul_mod(
        params->g2.pow_mod(this->contents, params->modulus), params->modulus);
}
...
CommitmentProofOfKnowledge::CommitmentProofOfKnowledge(const IntegerGroupParams* ap,
                                                       const IntegerGroupParams* bp,
                                                       const CBigNum& agIn,
                                                       const CBigNum& ahIn,
                                                       const CBigNum& bgIn,
                                                       const CBigNum& bhIn):
    ap(ap), bp(bp), ag(agIn), ah(ahIn), bg(bgIn), bh(bhIn) {}
...
CommitmentProofOfKnowledge::CommitmentProofOfKnowledge(const IntegerGroupParams* aParams,
                                                       ...
                                                       const CBigNum& bgIn,
                                                       const CBigNum& bhIn):
    ap(aParams), bp(bParams), ag(agIn), ah(ahIn), bg(bgIn), bh(bhIn)
{
	CBigNum r1, r2, r3;
  ...
  CBigNum T1 = this->ag.pow_mod(r1, this->ap->modulus).mul_mod((this->ah.pow_mod(r2, this->ap->modulus)), this->ap->modulus);
  CBigNum T2 = this->bg.pow_mod(r1, this->bp->modulus).mul_mod((this->bh.pow_mod(r3, this->bp->modulus)), this->bp->modulus);
...
bool CommitmentProofOfKnowledge::Verify(const CBigNum& A, const CBigNum& B) const
{
	...
	CBigNum T1 = A.pow_mod(this->challenge, ap->modulus).inverse(ap->modulus).mul_mod(
                  (ag.pow_mod(S1, ap->modulus).mul_mod(ah.pow_mod(S2, ap->modulus), ap->modulus)),
	                ap->modulus);
	...
	CBigNum T2 = B.pow_mod(this->challenge, bp->modulus).inverse(bp->modulus).mul_mod(
                  (bg.pow_mod(S1, bp->modulus).mul_mod(bh.pow_mod(S3, bp->modulus), bp->modulus)),
	                bp->modulus);
