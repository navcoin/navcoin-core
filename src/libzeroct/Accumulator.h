...
public:
    ...
    template<typename Stream>
    Accumulator(const ZeroCTParams* p, Stream& strm) {
        strm >> *this;
     ...
     * @throw     Zerocoin exception in case of invalid parameters
     **/
    Accumulator(const ZeroCTParams* p, CBigNum bnValue = 0);
...
public:
    AccumulatorWitness(const ZeroCTParams* p) : witness(&p->accumulatorParams), element(p) { }

    template<typename Stream>
    AccumulatorWitness(const ZeroCTParams* p, Stream& strm) {
        strm >> *this;
    }
     ...
     */
    AccumulatorWitness(const ZeroCTParams* p, const Accumulator& checkpoint, const PublicCoin coin);
