    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return DeVaultUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, DeVaultUnits::separatorNever);
            else
                return DeVaultUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != NULL) ? tr("Requested") + " ("+DeVaultUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
