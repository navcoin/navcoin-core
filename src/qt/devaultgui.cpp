...
const std::string DeVaultGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString DeVaultGUI::DEFAULT_WALLET = "~Default";

DeVaultGUI::DeVaultGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void DeVaultGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/devault"), tr("&Overview"), this);
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and devault: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your DeVault addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified DeVault addresses"));
    ...
    openAction->setStatusTip(tr("Open a devault: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible DeVault command-line options").arg(tr(PACKAGE_NAME)));
...
void DeVaultGUI::bootstrapBlockchain()
{
    bool ok = false;
    QString defaultUrl = "https://s3.amazonaws.com/devault-bootstrap/bootstrap-devault_" +
            QString::fromStdString(Params().NetworkIDString()) + "net.tar";
    QString url = QInputDialog::getText(this, tr("Bootstrap blockchain"),
                                            tr("You can use an external trusted source to download the blockchain from.<BR>The following URL points to a bootstrap copy provided by the DeVault Core Team.<BR>Where would you like to download it from?"), QLineEdit::Normal,
                                            defaultUrl, &ok);
    if (ok && !url.isEmpty())
    {
...
void DeVaultGUI::createMenuBar()
{
    ...
    if(walletFrame)
    {
        ...
        {
            QAction *menuAction = new QAction(QString(DeVaultUnits::name(u)), this);
...
void DeVaultGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void DeVaultGUI::createToolBars()
{
    if(walletFrame)
    {
...
void DeVaultGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool DeVaultGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void DeVaultGUI::startVotingCounter()
{
    if (GetStaking())
    {
...
bool DeVaultGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void DeVaultGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void DeVaultGUI::setWalletActionsEnabled(bool enabled)
{
...
void DeVaultGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void DeVaultGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void DeVaultGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void DeVaultGUI::optionsClicked()
{
...
void DeVaultGUI::cfundProposalsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
...
void DeVaultGUI::cfundProposalsOpen(bool fMode)
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
...
void DeVaultGUI::cfundPaymentRequestsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
...
void DeVaultGUI::showDebugWindow()
{
...
void DeVaultGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void DeVaultGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void DeVaultGUI::openClicked()
{
...
void DeVaultGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void DeVaultGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void DeVaultGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void DeVaultGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void DeVaultGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void DeVaultGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void DeVaultGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void DeVaultGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to DeVault network", "", count));
    if(walletFrame){
...
void DeVaultGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to DeVault network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to DeVault network..."));
...
void DeVaultGUI::showVotingDialog()
{
...
void DeVaultGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("DeVault"); // default title
...
void DeVaultGUI::changeEvent(QEvent *e)
{
...
void DeVaultGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void DeVaultGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void DeVaultGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(DeVaultUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void DeVaultGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void DeVaultGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool DeVaultGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool DeVaultGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void DeVaultGUI::setEncryptionStatus(int status)
{
...
void DeVaultGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void DeVaultGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void DeVaultGUI::detectShutdown()
{
...
void DeVaultGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void DeVaultGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(DeVaultGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void DeVaultGUI::subscribeToCoreSignals()
{
...
void DeVaultGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<DeVaultUnits::Unit> units = DeVaultUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const DeVaultUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(DeVaultUnits::name(unit)));
    }
...
{
    setText(DeVaultUnits::name(newUnits));
}
...
void DeVaultGUI::replyFinished(QNetworkReply *reply)
{
...
void DeVaultGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CDeVaultAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void DeVaultGUI::updateStakingStatus()
{
    updateWeight();
