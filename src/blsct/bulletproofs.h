...
public:
    ...
    static Generators GetGenerators(const std::pair<uint256,uint64_t>& tokenId=std::make_pair(uint256(),-1));

    void Prove(std::vector<Scalar> v, bls::G1Element nonce, const std::vector<uint8_t>& message = std::vector<uint8_t>(), const std::pair<uint256,uint64_t>& tokenId=std::make_pair(uint256(),-1), const std::vector<Scalar>& useGammas=std::vector<Scalar>());
    ...
    static std::map<std::pair<uint256, uint64_t>, bls::G1Element> H;
...
bool VerifyBulletproof(const std::vector<std::pair<int, BulletproofsRangeproof>>& proofs, std::vector<RangeproofEncodedData>& data, const std::vector<bls::G1Element>& nonces, const bool &fOnlyRecover = false, const std::pair<uint256,uint64_t>& tokenId=std::make_pair(uint256(),-1));
