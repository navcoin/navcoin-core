 ...
 *
 * See https://navcointalk.org/index.php?topic=8392.msg127623#msg127623
 *
        ...
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKSIGVERIFY:
                {
                    ...
                    if ((flags & SCRIPT_VERIFY_STRICTENC) && (!CheckSignatureEncoding(vchSig, flags, NULL) || !CheckPubKeyEncoding(vchPubKey, flags, NULL)))
                        return false;

                    bool fSuccess = CheckSignatureEncoding(vchSig, flags, NULL) && CheckPubKeyEncoding(vchPubKey, flags, NULL) &&
...
bool EvalScriptNavcoin(vector<vector<unsigned char> >& stack, const CScript& script, const CTransaction& txTo, const BaseSignatureCheckerNavcoin& checker, unsigned int nIn, unsigned int flags, int nHashType)
{
    static const CScriptNum bnZero(0);
    static const CScriptNum bnOne(1);
    static const CScriptNum bnFalse(0);
        ...
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKSIGVERIFY:
                {
                    ...
                    if (!CheckSignatureEncoding(vchSig, flags, NULL) || !CheckPubKeyEncoding(vchPubKey, flags, NULL)) {
                        //serror is set
                        return false;
                    }
...
bool EvalScriptNavcoin(vector<vector<unsigned char> >& stack, const CScript& script, unsigned int flags, const BaseSignatureCheckerNavcoin& checker, ScriptError* serror)
{
    static const CScriptNum bnZero(0);
    static const CScriptNum bnOne(1);
    static const CScriptNum bnFalse(0);
