...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total devault balance of the wallet\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"devaultaddress\"\n"
            "\nReturn information about the given devault address.\n"
            "\nArguments:\n"
            "1. \"devaultaddress\"     (string, required) The devault address to validate. This may also be an OpenAlias address.\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"devaultaddress\", (string) The devault address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"stakingaddress\" : \"devaultaddress\", (string) The devault staking address part of a cold staking address\n"
            "  \"spendingaddress\" : \"devaultaddress\", (string) The devault spending address part of a cold staking address\n"
    ...
    if (isValid)
    {
        ...
        if (address.IsColdStakingAddress(Params())) {
            CDeVaultAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CDeVaultAddress spendingAddress;
        ...
        if (address.IsPrivateAddress(Params())) {
            ...
            ret.push_back(Pair("ismine", CDeVaultAddress(pa) == address ? true :false));
        } else {
...
#ifdef ENABLE_WALLET
        // Case 1: DeVault address and we have full public key:
        CDeVaultAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are devault addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) devault address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CDeVaultAddress address(innerID);
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CDeVaultAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"devaultaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"devaultaddress\"  (string, required) The devault address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CDeVaultAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CDeVaultAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CDeVaultAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
