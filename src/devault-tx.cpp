...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s devault-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  devault-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded devault transaction") + "\n" +
              "  devault-tx [options] -create [commands]   " + _("Create hex-encoded devault transaction") + "\n" +
              "\n";
