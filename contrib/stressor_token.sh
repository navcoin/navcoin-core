...
function check_token_hash {
        ...
        for i in ${local_array[@]};
        do
		local_array_tokenhash[$i]=$(nav_cli $i "listtokens" | jq -c '[.[]|del(.balance)|del(.nfts)]'|shasum|awk '{print $1}')
        done
#	echo 'token hashes = ' ${local_array_tokenhash[@]}
        ...
        then
                ...
                then
                        ...
                        wait_until_sync "${local_array[@]}"
                        check_token_hash "${local_array[@]}"
                else
                        echo TOKEN HASH MISMATCH!
                        for i in ${!local_array_tokenhash[@]};
                        do
                                echo the hashes of nodes $i are ${local_array_tokenhash[$i]}
                        done
                        terminate 1
                fi
        fi
