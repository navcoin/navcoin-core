...
The RPM spec file provided here is for NavCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://navcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[NavCoin git repository](https://github.com/navcoin/navcoin) but are not part of
...
Currently only `Source10` is used, for the example `navcoin.conf` file.
...
in the navcoin git repository. At present only an SVG version of the NavCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the navcoin
...
needed when 0.12.1 is released, a proper fix is already in the NavCoin git
master branch.
...
idea to build NavCoin Core against a static version of that library compiled
...
The `navcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `navcoin-server` package. At this point, most RPM based
...
The `navcoin-server` package should have SELinux support. How to properly do
...
NavCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/navcoin/). He has been
packaging NavCoin for Fedora at least since 2012.
...
4. I renamed the `navcoin` package that contains the Qt GUI to `navcoin-core` as
that appears to be how the general population refers to it, in contrast to
`navcoin-xt` or `navcoin-classic`. I wanted to make sure the general population
