...
The RPM spec file provided here is for DeVault-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://devault.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[DeVault git repository](https://github.com/devault/devault) but are not part of
...
Currently only `Source10` is used, for the example `devault.conf` file.
...
in the devault git repository. At present only an SVG version of the DeVault
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the devault
...
needed when 0.12.1 is released, a proper fix is already in the DeVault git
master branch.
...
idea to build DeVault Core against a static version of that library compiled
...
The `devault.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `devault-server` package. At this point, most RPM based
...
The `devault-server` package should have SELinux support. How to properly do
...
DeVault-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/devault/). He has been
packaging DeVault for Fedora at least since 2012.
...
4. I renamed the `devault` package that contains the Qt GUI to `devault-core` as
that appears to be how the general population refers to it, in contrast to
`devault-xt` or `devault-classic`. I wanted to make sure the general population
